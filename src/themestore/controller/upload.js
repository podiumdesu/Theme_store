  ...
  async themeAction(){
    ...
    if(!aa) return this.fail(1000,'The necessary files are missing!');
      ...
      if(currentUser.user_uid === theme.theme_authoruid) {
        let insertId = await themeList.where({theme_name:themeName}).update({theme_authorname:currentUser.user_name,theme_version:jsonObj.version,theme_tags:jsonObj.tags,
        theme_description:jsonObj.description,theme_lastupdated:myDate.toLocaleDateString()});
