    ...
    if(originalFilename.match(/[a-zA-z_0-9_]*\.zip/)){
      ...
      //else {
        ...
        if(currentUser.user_uid === theme.theme_authoruid) {
          let insertId = await themeList.where({theme_name:name}).update({theme_authorname:currentUser.user_name,theme_version:jsonObj.version,theme_tags:jsonObj.tags,
